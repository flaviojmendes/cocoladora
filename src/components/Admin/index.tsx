...
export function AdminPage() {
  ...
  const filteredLocations = useMemo(() => {
    ...
    return locations.filter((item) =>
      [item.id, item.city, item.totalearned, item.day, item.timestarted, item.timeended]
        .join(" ")
        .toLowerCase()
        .includes(q)
    );
  ...
  const filteredPlaces = useMemo(() => {
    ...
    return places.filter((item) =>
      [item.id, item.name, item.notes.join(" ")].join(" ").toLowerCase().includes(q)
    );
  }, [places, query]);
  ...
  const filteredMessages = useMemo(() => {
    ...
    return messages.filter((item) =>
      [item.id, item.message].join(" ").toLowerCase().includes(q)
    );
  }, [messages, query]);
  ...
  const toggleSelectPage = () => {
    const ids = paged.map((row) => String((row as any).id));
                ...
                <thead className="bg-background-dark text-primary-dark">
                  <tr>
                    <th className="p-2 w-10">
                      <input type="checkbox" onChange={toggleSelectPage} checked={paged.length > 0 && paged.every((r) => selected.has(String((r as AdminLocation).id)))} />
                    </th>
                    <th className="p-2 text-left">ID</th>
                    <th className="p-2 text-left">{translate("adminCity")}</th>
                    <th className="p-2 text-left">{translate("adminEarned")}</th>
                    <th className="p-2 text-left">{translate("adminDay")}</th>
                    <th className="p-2 text-left">{translate("adminCoords")}</th>
                    <th className="p-2 text-right">{translate("adminActions")}</th>
                  </tr>
                </thead>
                <tbody>
                  {(paged as AdminLocation[]).map((item) => (
                    <tr key={item.id} className="border-t border-primary/10 hover:bg-amber-50/50">
                      <td className="p-2">
                        <input type="checkbox" checked={selected.has(String(item.id))} onChange={() => toggleSelect(String(item.id))} />
                      </td>
                      ...
                      <td className="p-2">{item.day} {item.timestarted}-{item.timeended}</td>
                      <td className="p-2 font-typewriter text-xs">{item.latitude.toFixed(4)}, {item.longitude.toFixed(4)}</td>
                      <td className="p-2 text-right whitespace-nowrap">
                        <button type="button" className="text-primary px-2" onClick={() => setEditingLocation({ ...item })} aria-label={translate("adminEdit")}>
                          <FaEdit />
                        </button>
                        <button type="button" className="text-red-700 px-2" onClick={() => deleteIds([String(item.id)])} aria-label={translate("adminDelete")}>
                          <FaTrash />
                        </button>
                      </td>
                    </tr>
            ...
            {tab === "places" && (
              <table className="w-full text-sm font-secondary min-w-[720px]">
                <thead className="bg-background-dark text-primary-dark">
                  <tr>
                    <th className="p-2 w-10">
                      <input type="checkbox" onChange={toggleSelectPage} checked={paged.length > 0 && paged.every((r) => selected.has(String((r as AdminPlace).id)))} />
                    </th>
                    <th className="p-2 text-left">{translate("adminName")}</th>
                    <th className="p-2 text-left">{translate("adminRatings")}</th>
                    <th className="p-2 text-left">{translate("adminCoords")}</th>
                    <th className="p-2 text-right">{translate("adminActions")}</th>
                  </tr>
                </thead>
                <tbody>
                  {(paged as AdminPlace[]).map((item) => (
                    <tr key={item.id} className="border-t border-primary/10 hover:bg-amber-50/50">
                      ...
                      <td className="p-2">{item.cleanRating}/{item.facilitiesRating}/{item.privacyRating}</td>
                      <td className="p-2 font-typewriter text-xs">{item.latitude.toFixed(4)}, {item.longitude.toFixed(4)}</td>
                      <td className="p-2 text-right whitespace-nowrap">
                        <button type="button" className="text-primary px-2" onClick={() => setEditingPlace({ ...item, notes: [...item.notes] })}>
                          <FaEdit />
                        </button>
            ...
            {tab === "messages" && (
              <table className="w-full text-sm font-secondary min-w-[640px]">
                <thead className="bg-background-dark text-primary-dark">
                  <tr>
                    <th className="p-2 w-10">
                      <input type="checkbox" onChange={toggleSelectPage} checked={paged.length > 0 && paged.every((r) => selected.has(String((r as AdminMessage).id)))} />
                    </th>
                    <th className="p-2 text-left">{translate("adminMessage")}</th>
                    <th className="p-2 text-left">{translate("adminColor")}</th>
                    <th className="p-2 text-right">{translate("adminActions")}</th>
                  </tr>
                </thead>
                <tbody>
                  {(paged as AdminMessage[]).map((item) => (
                    <tr key={item.id} className="border-t border-primary/10 hover:bg-amber-50/50">
                      ...
                      <td className="p-2 font-typewriter text-xs">{item.fontColor} · {item.rotation}°</td>
                      <td className="p-2 text-right whitespace-nowrap">
        ...
        {editingLocation && (
          <form onSubmit={saveLocation} className="flex flex-col gap-3">
            <label className="font-secondary text-sm">
              {translate("adminCity")}
              <input className={`${inputClass} mt-1`} value={editingLocation.city} onChange={(e) => setEditingLocation({ ...editingLocation, city: e.target.value })} />
            </label>
            <label className="font-secondary text-sm">
              {translate("adminEarned")}
              <input className={`${inputClass} mt-1`} value={editingLocation.totalearned} onChange={(e) => setEditingLocation({ ...editingLocation, totalearned: e.target.value })} />
            </label>
            <div className="grid grid-cols-3 gap-2">
              <label className="font-secondary text-sm">
                {translate("adminDay")}
                <input className={`${inputClass} mt-1`} value={editingLocation.day} onChange={(e) => setEditingLocation({ ...editingLocation, day: e.target.value })} />
              </label>
              <label className="font-secondary text-sm">
                {translate("start")}
                <input className={`${inputClass} mt-1`} value={editingLocation.timestarted} onChange={(e) => setEditingLocation({ ...editingLocation, timestarted: e.target.value })} />
              </label>
              <label className="font-secondary text-sm">
                {translate("end")}
                <input className={`${inputClass} mt-1`} value={editingLocation.timeended} onChange={(e) => setEditingLocation({ ...editingLocation, timeended: e.target.value })} />
              </label>
            </div>
            <div className="grid grid-cols-2 gap-2">
              <label className="font-secondary text-sm">
                Lat
                <input type="number" step="any" className={`${inputClass} mt-1`} value={editingLocation.latitude} onChange={(e) => setEditingLocation({ ...editingLocation, latitude: parseFloat(e.target.value) })} />
              </label>
              <label className="font-secondary text-sm">
                Lng
                <input type="number" step="any" className={`${inputClass} mt-1`} value={editingLocation.longitude} onChange={(e) => setEditingLocation({ ...editingLocation, longitude: parseFloat(e.target.value) })} />
              </label>
            </div>
            <button type="submit" disabled={saving} className="mt-2 py-2.5 rounded-xl bg-primary text-background font-primary text-xl font-bold">
      ...
      >
        {editingPlace && (
          <form onSubmit={savePlace} className="flex flex-col gap-3">
            <label className="font-secondary text-sm">
              {translate("adminName")}
              <input className={`${inputClass} mt-1`} value={editingPlace.name} onChange={(e) => setEditingPlace({ ...editingPlace, name: e.target.value })} />
            </label>
            <div className="grid grid-cols-3 gap-2">
              <label className="font-secondary text-sm">
                {translate("cleaness")}
                <input type="number" min={0} max={5} className={`${inputClass} mt-1`} value={editingPlace.cleanRating} onChange={(e) => setEditingPlace({ ...editingPlace, cleanRating: parseInt(e.target.value, 10) || 0 })} />
              </label>
              <label className="font-secondary text-sm">
                {translate("facilities")}
                <input type="number" min={0} max={5} className={`${inputClass} mt-1`} value={editingPlace.facilitiesRating} onChange={(e) => setEditingPlace({ ...editingPlace, facilitiesRating: parseInt(e.target.value, 10) || 0 })} />
              </label>
              <label className="font-secondary text-sm">
                {translate("privacy")}
                <input type="number" min={0} max={5} className={`${inputClass} mt-1`} value={editingPlace.privacyRating} onChange={(e) => setEditingPlace({ ...editingPlace, privacyRating: parseInt(e.target.value, 10) || 0 })} />
              </label>
            </div>
            <div className="grid grid-cols-2 gap-2">
              <label className="font-secondary text-sm">
                Lat
                <input type="number" step="any" className={`${inputClass} mt-1`} value={editingPlace.latitude} onChange={(e) => setEditingPlace({ ...editingPlace, latitude: parseFloat(e.target.value) })} />
              </label>
              <label className="font-secondary text-sm">
                Lng
                <input type="number" step="any" className={`${inputClass} mt-1`} value={editingPlace.longitude} onChange={(e) => setEditingPlace({ ...editingPlace, longitude: parseFloat(e.target.value) })} />
              </label>
            </div>
            <label className="font-secondary text-sm">
          ...
          <form onSubmit={saveMessage} className="flex flex-col gap-3">
            ...
            <div className="grid grid-cols-2 gap-2">
              <label className="font-secondary text-sm">
                ...
                <input type="color" className={`${inputClass} mt-1 h-11`} value={editingMessage.fontColor} onChange={(e) => setEditingMessage({ ...editingMessage, fontColor: e.target.value })} />
              </label>
              <label className="font-secondary text-sm">
                {translate("adminRotation")}
                <input type="number" step="0.1" className={`${inputClass} mt-1`} value={editingMessage.rotation} onChange={(e) => setEditingMessage({ ...editingMessage, rotation: parseFloat(e.target.value) || 0 })} />
              </label>
            </div>
            <button type="submit" disabled={saving} className="mt-2 py-2.5 rounded-xl bg-primary text-background font-primary text-xl font-bold">
              {saving ? translate("adminSaving") : translate("adminSave")}
